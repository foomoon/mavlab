function K = assemble(trinodes,trielements,Edof,varargin),
% ASSEMBLE - Compute Stiffness Matrix for Mav
%   
%   [K] = ASSEMBLE(nodes,tri,edof,properties) returns the
%   stiffness matrix (K) for the wing geometry
%   
%   nodes      - nodal values (n-by-3)
%   tri        - element array (n-by_4) *4th column is element type
%   edof       - element degree of freedom (calculated by connectivity)
...
for i = 1:N_elements,          
    switch element_type(i),                
        ...
        Ke = plateElement(Dp,X1(i),X2(i),X3(i),Y1(i),Y2(i),Y3(i));       
        case 1,
        %% Calculate stiffness matrix of each latex element        
        Ke = membraneElement(T,X1(i),X2(i),X3(i),Y1(i),Y2(i),Y3(i));        
        case 2,
        %% Calculate stiffness matrix of each batten element
        Ke = plateElement(Db,X1(i),X2(i),X3(i),Y1(i),Y2(i),Y3(i));        
    end               
