function gnrbcurv(mode,res)
% GNRBCURV  Interactive manipulation of NURBS space curve
%
%    For use with the NURBS toolbox, GNRBCURV allows the user to have an
%    intuitive manipulation of a NURBS curve via control points.  
%
%    GNRBCURV with no inputs will generate a test surface for manipulation.
%    GNRBCURV(NRB), where NRB is a structure defining a NURBS curve (as
%    defined in the NURBS toolbox).
%    GNRBCURV(NRB,RES), where RES is a 1 element vector defining the
%    resolution of curve rendering in the u direction, 
%    ie RES = [URES,VRES].  The default is [100] if no argument is
%    specified.
%    
...
if ~nargin
    mode = 'start';
    nrb = nrbtestcrv; % Default Surface
    res = 100; % Default Resolution
...
%     disp(mode)
    ...
    error('GNRBCURV: Wrong type of input')
end
...
switch mode
    case 'start'
        ...
        set(hnet, 'ButtonDownFcn','gnrbcurv(''click'')');
    ...
    case 'click'        
        ...
        set(fig,'WindowButtonMotionFcn','gnrbcurv(''move'')');
        set(fig,'WindowButtonUpFcn','gnrbcurv(''up'')');
        set(fig,'UserData',ud);
    ...
    otherwise
        error('gnrbcurv: Wrong input string')
end
