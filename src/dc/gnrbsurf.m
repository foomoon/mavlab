function out = gnrbsurf(mode,res)
% GNRBSURF  Interactive manipulation of NURBS surface
%
%    For use with the NURBS toolbox, GNRBSURF allows the user to have an
%    intuitive manipulation of a NURBS surface via control points.  
%
%    GNRBSURF with no inputs will generate a test surface for manipulation.
%    GNRBSURF(NRB), where NRB is a structure defining a NURBS surface (as
%    defined in the NURBS toolbox).
%    GNRBSURF(NRB,RES), where RES is a 2 element vector defines the
%    resolution of surface rendering in the u and v directions, 
...
%     disp(mode)
    ...
    error('GNRBSURF: Wrong type of input')
end
...
switch mode
    case 'start'
        ...
        set(hnet, 'ButtonDownFcn','gnrbsurf(''click'')');
    ...
    case 'click'        
        ...
        set(fig,'WindowButtonMotionFcn','gnrbsurf(''move'')');
        set(fig,'WindowButtonUpFcn','gnrbsurf(''up'')');
        set(fig,'UserData',ud);
    ...
    otherwise
        error('GNRBSURF: Wrong input string')
end
